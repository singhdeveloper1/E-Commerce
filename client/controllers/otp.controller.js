...
export const getOTP = async (req, res, next) => {
  ...
  if (!phone && !email)
    return next(errorHandler(400, "email or phone is required to send otp"));

  const existingAccount = await User.findOne({ $or: [{ email, phone }] });

  if (!existingAccount)
    return next(errorHandler(401, "email or phone not exist!!"));
  ...
  sendMail(
    email,
    "One Time Password!!",
    `this is the OTP <h2>${otp}</h2 for your email and its only valid for <h3>5 Min</h3>`
  );
