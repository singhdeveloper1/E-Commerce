    ...
    if (!cart) {
      ...
      cart.products.map((product) => {
        if (product.productId == req.params.productId)
          return next(errorHandler(400, "already added in cart"));
      });
      cart.products.push({
        productId: req.params.productId,
        quantity,
...
export const viewCart = async (req, res, next) => {
  try {
    const view = await AddToCart.findOne({ userId: req.user._id }).populate(
      "products.productId"
    );

    if (!view || view.length == 0)
      return next(errorHandler(404, "No product added in the cart"));
...
export const updateCartQuantity = async (req, res, next) => {
  ...
  try {
    ...
    products.forEach((item) => {
      const update = cart.products.find(
...
export const deleteOneCart = async (req, res, next) => {
  try {
    const user = await AddToCart.findOneAndUpdate(
      { userId: req.user._id },
      {
