...
export const switchToUser = async (req, res, next) => {
  try {
    if (!req.user.isSeller)
      return next(errorHandler(401, "you are already a user!!!"));
  ...
  try {
    ...
    if (req.files) {
      ...
      for (const file of req.files) {
        const result = await uploadToClodinary(
          file.path,
          "Product_Images",
          next
        );
    ...
    if (imageUrl.length == 0) {
      imageUrl.push(
        "https://odoo-community.org/web/image/product.template/1844/image_1024?unique=1e911c3"
      );
    }
...
export const updateCategoryImage = async (req, res, next) => {
  ...
  try {
    await CategoryImage.findByIdAndUpdate(
      req.params.id,
      {
        image: imageUrl,
      },
...
export const addVariant = async (req, res, next) => {
  ...
  try {
    ...
    const sameColorVariant = await Variant.findOne({
      productId: req.params.productId,
      color,
    });
    if (sameColorVariant) {
...
export const updateDeliveryStatus = async (req, res, next) => {
  ...
  try {
    ...
    if (singleOrder.isCancel || singleOrder.isReturn) {
      return next(
        errorHandler(401, "this order was either cancelled or returned!!")
      );
    }
