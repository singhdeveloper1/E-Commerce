...
export const applyCoupon = async (req, res, next) => {
  ...
  try {
    ...
    if (!existingCoupon)
      return next(errorHandler(404, "Coupon does not exist"));

    if (existingCoupon.expiresAt < new Date())
      return next(errorHandler(400, "Coupon was expired..."));
    ...
    if (existingCoupon.isForNewUser && previousOrder.length > 0)
      return next(
        errorHandler(400, "This coupon is only valid for first order...")
      );

    //! based on product and category

    let eligibleItems = cartItems;

    const validProductAndCategory =
      (existingCoupon.validProducts &&
        existingCoupon.validProducts.length > 0) ||
      (existingCoupon.validCategories &&
        existingCoupon.validCategories.length > 0);
    ...
    if (validProductAndCategory) {
      eligibleItems = cartItems.filter((item) => {
        ...
        const productMatch = existingCoupon.validProducts?.includes(
          item.productId
        );
        const categoryMatch = existingCoupon.validCategories?.includes(
          item.category
        );
      ...
      if (eligibleItems.length == 0)
        return next(
          errorHandler(
            400,
            "this coupon is only valid for specifc products or sprcific category!!!"
          )
        );
    }
    ...
    if (existingCoupon.discountType === "Percentage") {
      ...
      discountedPrice =
        eligibleTotal - discountToApply * eligibleItems.length + finalPrice;
    }
